...
ALTER TABLE case_agent_events ADD CONSTRAINT case_agent_events_event_type_check CHECK (event_type IN (
 ...
 'SUPPLEMENTARY_MATERIAL_STAGE_REVIEWED','CASE_ANALYSIS_STAGE_REVIEWED','PLANNING_RESULT_UNKNOWN','TASK_GRAPH_ACCEPTED','APPROVAL_GRANTED',
...
CREATE FUNCTION guard_case_agent_case_analysis_stage() RETURNS trigger
...
BEGIN
 IF NEW.event_type <> 'CASE_ANALYSIS_STAGE_REVIEWED' THEN RETURN NEW; END IF;
 ...
 END IF;
 -- A successful extraction must precede this transition; never renew an analysis retry.
 IF NOT EXISTS (SELECT 1 FROM public.case_agent_tasks task
   WHERE task.run_id=NEW.run_id AND task.firm_id=NEW.firm_id AND task.matter_id=NEW.matter_id
    AND task.graph_id=head.current_graph_id AND task.tool_id='extract_case_ledger') THEN
  RAISE EXCEPTION 'analysis continuation has no preceding extraction';
 END IF;
...
END;
$$;
CREATE TRIGGER case_agent_case_analysis_stage_guard BEFORE INSERT ON case_agent_events
 FOR EACH ROW EXECUTE FUNCTION guard_case_agent_case_analysis_stage();

CREATE FUNCTION public.case_agent_is_bounded_case_analysis_stage(p_run_id uuid,p_firm_id uuid,p_matter_id uuid)
 ...
 SELECT EXISTS (
  ...
  JOIN LATERAL (SELECT payload->'stage' AS stage FROM public.case_agent_events
   WHERE run_id=r.run_id AND firm_id=r.firm_id AND matter_id=r.matter_id
    AND event_type='CASE_ANALYSIS_STAGE_REVIEWED'
...
$$;
REVOKE ALL ON FUNCTION public.case_agent_is_bounded_case_analysis_stage(uuid,uuid,uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.case_agent_is_bounded_case_analysis_stage(uuid,uuid,uuid)
...
BEGIN
 ...
 IF strpos(original,'SELECT public.case_agent_is_bounded_supplementary_material_stage(p_run_id,p_firm_id,p_matter_id) OR EXISTS (')=0 OR strpos(original,'case_agent_is_bounded_case_analysis_stage')>0 THEN
  RAISE EXCEPTION 'pending review function differs';
 END IF;
 EXECUTE replace(original,'SELECT public.case_agent_is_bounded_supplementary_material_stage(p_run_id,p_firm_id,p_matter_id) OR EXISTS (',
  'SELECT public.case_agent_is_bounded_case_analysis_stage(p_run_id,p_firm_id,p_matter_id) OR public.case_agent_is_bounded_supplementary_material_stage(p_run_id,p_firm_id,p_matter_id) OR EXISTS (');
END;
$patch$;
COMMIT;
