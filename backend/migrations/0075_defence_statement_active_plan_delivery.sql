-- Add the narrow defendant-response candidate to the governed Agent catalogue.
--
-- This changes only the closed JSON shapes enforced on goal and ACTIVE-plan
-- execution records.  It neither creates a plan nor relaxes the lawyer
-- activation, source-binding, document review, submission, or audit gates.
...
BEGIN
    ...
    IF top_keys <> ARRAY['items','plan_hash','plan_id','source_run_id']::text[]
       ...
       OR jsonb_array_length(execution->'items') NOT BETWEEN 1 AND 3 THEN
        RETURN false;
    END IF;
    FOR item IN
        SELECT value
    ...
    LOOP
        ...
        IF item_keys <>
           ...
           OR coalesce(item->>'deliverable_kind', '') NOT IN
              ('CASE_REVIEW_MEMO', 'DEFENCE_STATEMENT', 'PAYMENT_LEDGER') THEN
            RETURN false;
        END IF;
    END LOOP;
