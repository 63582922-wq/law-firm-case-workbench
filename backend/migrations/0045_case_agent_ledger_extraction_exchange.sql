-- Recoverable, one-call DeepSeek exchange for review-only ledger extraction.
--
-- The generic 0031 STARTED row is committed before this immutable exchange
-- can be inserted.  Raw responses are held only in encrypted private object
-- storage; PostgreSQL retains exact request/task/schema hashes and the private
-- locator needed for lookup-only recovery.  No prompt, API key, case text or
-- provider bytes are stored here.

BEGIN;

CREATE TABLE case_agent_ledger_extraction_exchanges (
    ...
    input_refs_hash char(64) NOT NULL CHECK (input_refs_hash ~ '^[0-9a-f]{64}$'),
    ...
    service_id text NOT NULL CHECK (service_id = 'deepseek-ledger-extraction'),
    model_id text NOT NULL CHECK (model_id = 'deepseek-v4-pro'),
    response_schema_id text NOT NULL CHECK (
        response_schema_id = 'deepseek-case-ledger-extraction-response-v1'
    ),
    response_schema_hash char(64) NOT NULL CHECK (
        response_schema_hash =
        'e7fbbde0a78e04623cb0281e8fa39ae55912ed98ae44b4288f45abcd355967c2'
    ),
...
CREATE TABLE case_agent_ledger_extraction_outcomes (
    ...
    provider_response_id_hash char(64) CHECK (
        provider_response_id_hash ~ '^[0-9a-f]{64}$'
    ),
    response_sha256 char(64) CHECK (response_sha256 ~ '^[0-9a-f]{64}$'),
    response_bytes integer CHECK (response_bytes BETWEEN 2 AND 8388608),
    response_object_key text CHECK (
        response_object_key IS NULL OR response_object_key ~
        '^case-agent-ledger-extractions/v1/[0-9a-f-]{36}/[0-9a-f-]{36}/[0-9a-f-]{36}/[0-9a-f]{64}\.json$'
    ),
    ...
    CHECK (
        ...
        (status = 'UNKNOWN_SUBMISSION'
         ...
         AND error_code = 'LEDGER_EXTRACTION_OUTCOME_UNKNOWN')
    ),
    CHECK (
        (outcome_sequence = 1 AND recovered_from_unknown = false)
        OR
        (outcome_sequence = 2 AND recovered_from_unknown = true
...
COMMENT ON TABLE case_agent_ledger_extraction_exchanges IS
    'Immutable 0031-bound DeepSeek ledger extraction request; no prompt, key, case text or provider response bytes.';
COMMENT ON TABLE case_agent_ledger_extraction_outcomes IS
    'Append-only one-call result ledger. Raw provider bytes remain only in encrypted private object storage.';
COMMENT ON COLUMN case_agent_ledger_extraction_outcomes.response_object_key IS
    'Private server locator; forbidden from browser, audit payload and ordinary logs.';

CREATE INDEX case_agent_ledger_extraction_exchanges_matter_idx
    ON case_agent_ledger_extraction_exchanges(
        firm_id, matter_id, run_id, task_id, started_at
    );
CREATE INDEX case_agent_ledger_extraction_outcomes_latest_idx
    ON case_agent_ledger_extraction_outcomes(
        firm_id, external_request_id, outcome_sequence DESC
    );

CREATE FUNCTION validate_case_agent_ledger_extraction_exchange()
...
BEGIN
    ...
    IF task_row IS NULL
       OR task_row.skill_id IS DISTINCT FROM 'case_ledger_extraction'
       OR task_row.skill_version IS DISTINCT FROM '1.0.0'
       OR task_row.tool_id IS DISTINCT FROM 'extract_case_ledger'
       OR task_row.tool_version IS DISTINCT FROM '1.0.0'
       OR task_row.adapter_id IS DISTINCT FROM 'deepseek-case-ledger-extraction'
       ...
       OR task_row.sandbox_policy_hash IS DISTINCT FROM
          'a42094b805caf40dc97e26a4b516a13a6e76671a7f53606881de121238142cac'
       ...
       OR task_row.is_stale OR task_row.is_cancelled THEN
        RAISE EXCEPTION 'ledger extraction exchange is not a current exact approved task';
    END IF;

    IF jsonb_array_length(task_row.input_refs) < 1 OR EXISTS (
        SELECT 1 FROM jsonb_array_elements_text(task_row.input_refs) AS ref(value)
         WHERE ref.value !~
         '^evidence-page:[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
    ) THEN
        RAISE EXCEPTION 'ledger extraction task inputs are not evidence pages';
    END IF;

    IF NEW.task_input_hash IS DISTINCT FROM task_row.input_hash
       OR NEW.input_refs_hash IS DISTINCT FROM encode(
            digest(convert_to(
                replace(task_row.input_refs::text, ', ', ','), 'UTF8'
            ), 'sha256'), 'hex'
       ) THEN
        -- Evidence refs contain no whitespace.  Removing PostgreSQL's JSONB
        -- separator space therefore produces the same compact canonical JSON
        -- array hashed by the Worker.
        RAISE EXCEPTION 'ledger extraction task input binding differs';
    END IF;
    ...
    IF submission_row IS NULL
       OR submission_row.external_request_id IS DISTINCT FROM NEW.external_request_id::text
       ...
       OR NOT EXISTS (
        ...
        RAISE EXCEPTION 'ledger extraction 0031 STARTED boundary is absent or differs';
    END IF;
    RETURN NEW;
END;
$$;

CREATE TRIGGER case_agent_ledger_extraction_exchange_guard
    BEFORE INSERT ON case_agent_ledger_extraction_exchanges
    FOR EACH ROW EXECUTE FUNCTION validate_case_agent_ledger_extraction_exchange();

CREATE FUNCTION validate_case_agent_ledger_extraction_outcome()
RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE
    exchange_row case_agent_ledger_extraction_exchanges%ROWTYPE;
BEGIN
    ...
    IF exchange_row.exchange_id IS NULL
       ...
       OR exchange_row.response_schema_hash IS DISTINCT FROM NEW.response_schema_hash
       OR exchange_row.started_by_worker IS DISTINCT FROM NEW.recorded_by_worker
       OR NULLIF(current_setting('app.actor_id', true), '')::uuid
          IS DISTINCT FROM NEW.recorded_by_worker THEN
        RAISE EXCEPTION 'ledger extraction outcome differs from its exchange';
    END IF;
    IF NEW.outcome_sequence = 2 AND NOT EXISTS (
        SELECT 1 FROM case_agent_ledger_extraction_outcomes prior
        ...
        RAISE EXCEPTION 'ledger extraction recovery requires one prior unknown outcome';
    END IF;
    RETURN NEW;
END;
$$;

CREATE TRIGGER case_agent_ledger_extraction_outcome_guard
    BEFORE INSERT ON case_agent_ledger_extraction_outcomes
    FOR EACH ROW EXECUTE FUNCTION validate_case_agent_ledger_extraction_outcome();

CREATE FUNCTION prohibit_case_agent_ledger_extraction_exchange_change()
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
    RAISE EXCEPTION 'ledger extraction exchange ledger is append-only';
END;
$$;

CREATE TRIGGER case_agent_ledger_extraction_exchanges_append_only
    BEFORE UPDATE OR DELETE ON case_agent_ledger_extraction_exchanges
    FOR EACH ROW EXECUTE FUNCTION prohibit_case_agent_ledger_extraction_exchange_change();
CREATE TRIGGER case_agent_ledger_extraction_outcomes_append_only
    BEFORE UPDATE OR DELETE ON case_agent_ledger_extraction_outcomes
    FOR EACH ROW EXECUTE FUNCTION prohibit_case_agent_ledger_extraction_exchange_change();

ALTER TABLE case_agent_ledger_extraction_exchanges ENABLE ROW LEVEL SECURITY;
ALTER TABLE case_agent_ledger_extraction_exchanges FORCE ROW LEVEL SECURITY;
CREATE POLICY case_agent_ledger_extraction_exchanges_firm_isolation
    ON case_agent_ledger_extraction_exchanges
    USING (firm_id::text = current_setting('app.firm_id', true))
    WITH CHECK (firm_id::text = current_setting('app.firm_id', true));

ALTER TABLE case_agent_ledger_extraction_outcomes ENABLE ROW LEVEL SECURITY;
ALTER TABLE case_agent_ledger_extraction_outcomes FORCE ROW LEVEL SECURITY;
CREATE POLICY case_agent_ledger_extraction_outcomes_firm_isolation
    ON case_agent_ledger_extraction_outcomes
    USING (firm_id::text = current_setting('app.firm_id', true))
    WITH CHECK (firm_id::text = current_setting('app.firm_id', true));

REVOKE ALL ON TABLE case_agent_ledger_extraction_exchanges FROM PUBLIC;
REVOKE ALL ON TABLE case_agent_ledger_extraction_outcomes FROM PUBLIC;
REVOKE ALL ON FUNCTION validate_case_agent_ledger_extraction_exchange() FROM PUBLIC;
REVOKE ALL ON FUNCTION validate_case_agent_ledger_extraction_outcome() FROM PUBLIC;
REVOKE ALL ON FUNCTION prohibit_case_agent_ledger_extraction_exchange_change() FROM PUBLIC;
