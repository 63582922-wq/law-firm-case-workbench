-- A verified analysis graph deliberately excludes raw evidence pages from the
-- first model packet.  When its requested deliverables include the
-- deterministic evidence catalogue, the work-plan compiler adds only the
-- already-confirmed pages that must become catalogue rows.  The original
-- 0043 trigger required a byte-for-byte graph-input match and therefore
-- rejected that safe, server-derived addition at commit time.
--
-- Keep the normal graph-to-binding equality rule.  The sole exception is a
-- confirmed evidence page which is also an exact SOURCE reference of this
-- candidate plan's EVIDENCE_CATALOGUE document item.  No other source type,
-- status, reference use, document kind, browser input, or model output is
-- admitted by this migration.
                ...
                AND NOT (
                    ...
                    AND EXISTS (
                        ...
                        WHERE item.plan_id = NEW.plan_id
                          ...
                          AND item_reference.source_id = binding.object_id
                          AND item_reference.source_version = binding.object_version
                          AND item_reference.source_hash = binding.content_hash
