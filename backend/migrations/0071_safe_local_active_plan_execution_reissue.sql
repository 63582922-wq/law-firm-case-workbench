-- PostgreSQL 16+; apply after 0070_local_case_review_memo_policy_hash.sql.
--
-- A verified ACTIVE plan must not require another paid planning/model run when
-- its deterministic document execution stops on a side-effect-free local
-- adapter or renderer failure.  This migration keeps execution history
-- append-only and allows one new, explicit lead-lawyer POST to create the next
-- execution attempt only when the immediately preceding run is WAITING_INPUT,
-- has a known local failure and has no external/unknown result.

BEGIN;

ALTER TABLE public.case_agent_active_plan_execution_runs
    ADD COLUMN execution_attempt integer NOT NULL DEFAULT 1
        CHECK (execution_attempt > 0),
    ADD COLUMN supersedes_execution_id uuid,
    ADD COLUMN retry_reason_code text,
    ADD CONSTRAINT case_agent_active_plan_execution_retry_shape CHECK (
        (
            execution_attempt = 1
            AND supersedes_execution_id IS NULL
            AND retry_reason_code IS NULL
        )
        OR (
            execution_attempt > 1
            AND supersedes_execution_id IS NOT NULL
            AND retry_reason_code = 'SAFE_LOCAL_FAILURE_REISSUE'
        )
    ),
    ADD CONSTRAINT case_agent_active_plan_execution_supersedes_fkey
        FOREIGN KEY (supersedes_execution_id)
        REFERENCES public.case_agent_active_plan_execution_runs(execution_id),
    ADD CONSTRAINT case_agent_active_plan_execution_supersedes_unique
        UNIQUE (supersedes_execution_id);

ALTER TABLE public.case_agent_active_plan_execution_runs
    DROP CONSTRAINT case_agent_active_plan_execution__plan_id_firm_id_matter_id_key,
    ADD CONSTRAINT case_agent_active_plan_execution_plan_attempt_unique
        UNIQUE (plan_id, firm_id, matter_id, execution_attempt);
...
CREATE INDEX case_agent_active_plan_execution_attempt_idx
    ON public.case_agent_active_plan_execution_runs(
        firm_id, matter_id, plan_id, execution_attempt DESC
    );
