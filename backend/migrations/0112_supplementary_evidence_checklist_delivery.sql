-- Add the internal, source-bound supplementary evidence checklist to the
-- closed deliverable catalogue.  It remains a lawyer-review candidate; this
-- migration grants no collection, approval, external communication, or court
...
BEGIN
    ...
    IF top_keys <> ARRAY['items','plan_hash','plan_id','source_run_id']::text[]
       ...
       OR jsonb_array_length(execution->'items') NOT BETWEEN 1 AND 5 THEN
        RETURN false;
    END IF;
    FOR item IN
        SELECT value
    ...
    LOOP
        ...
        IF item_keys <>
           ...
           OR (
                item->>'deliverable_kind' IN (
                    'CASE_REVIEW_MEMO', 'DEFENCE_STATEMENT',
                    'SUPPLEMENTARY_EVIDENCE_CHECKLIST'
                )
           ...
           OR coalesce(item->>'deliverable_kind', '') NOT IN (
                'CASE_REVIEW_MEMO', 'DEFENCE_STATEMENT',
                'EVIDENCE_CATALOGUE', 'SUPPLEMENTARY_EVIDENCE_CHECKLIST',
                'PAYMENT_LEDGER'
           ) THEN
            RETURN false;
        END IF;
