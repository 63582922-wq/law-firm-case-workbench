-- Preserve a controlled transport stage for indeterminate Qwen OCR requests.
-- The append-only outcome still remains UNKNOWN_SUBMISSION and cannot be
-- retried automatically.  No exception text, provider body or evidence bytes
-- are stored in the diagnostic receipt.
...
COMMENT ON CONSTRAINT case_agent_visual_ocr_outcomes_check
    ...
    'Unknown OCR requests retain only a controlled network stage and remain lookup-only; no automatic resend.';
