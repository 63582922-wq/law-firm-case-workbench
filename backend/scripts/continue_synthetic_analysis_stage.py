...
KEY = "unassisted-source-bound-analysis-stage-20260910-v1"
...
def main():
    if sys.argv[1:] == ["--check-plan"]:
        ...
        stage = prepare_case_analysis_stage(state=state, candidate_bindings=bindings, approved_by=REVIEWER)
    ...
    try:
        ...
        if sys.argv[1:] == ["--approve-analysis"]:
            ...
            if (state.event_version != 40 or state.analysis_stage is None
                    or state.budget_usage.external_calls != 2 or len(approvals) != 1):
        ...
        with store._read_transaction(identity.actor.firm_id) as connection:
            ...
            if existing is not None:
                if existing.event_type is not AgentEventType.CASE_ANALYSIS_STAGE_REVIEWED:
        ...
        if state.event_version != 33 or state.snapshot.matter_version != 14 or len(bindings) != 56:
