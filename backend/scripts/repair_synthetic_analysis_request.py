...
KEY = "unassisted-discovered-request-repair-20260910-v1"


def prepare_revision(*, state, candidate_bindings, approved_by):
    return prepare_case_analysis_request_repair(state=state, candidate_bindings=candidate_bindings,
        approved_by=approved_by, failed_attempt_id="ce2e7015-1704-5904-a1e8-443f4830369f",
        failed_request_hash="e5bcb8f9dfaf916e4a1389a6e9348fc6f4a7feb80b13443067f67ae861bad918",
        repaired_request_hash="ac36c3176fcc863638be02674b8b8c61952f1337be80b7aa8a72bb64801bf238",
        repair_reason="旧请求返回HTTP400且未产生成果。修复无核验法源时的空enum请求格式；保留原拒绝和累计用量，仅发送已核对的新请求一次。")
...
def main():
    ...
    try:
        ...
        if sys.argv[1:] == ["--approve-analysis"]:
            ...
            if (state.event_version != 62 or state.analysis_stage is None
                    or state.budget_usage.external_calls != 4 or len(approvals) != 1):
        ...
        with store._read_transaction(identity.actor.firm_id) as connection:
            ...
            if existing is not None:
                if existing.event_type is not AgentEventType.CASE_ANALYSIS_REQUEST_REPAIR_REVIEWED:
        ...
        if state.event_version != 55 or state.snapshot.matter_version != 14 or len(bindings) != 56:
        ...
        if sys.argv[1:] == ["--apply"]:
            receipt = store.review_case_analysis_stage(matter_id=MATTER, actor=identity.actor,
                ...
                event=AgentSupervisorEvent(event_id=event_id, run_id=RUN, firm_id=identity.actor.firm_id,
                    ...
                    event_type=AgentEventType.CASE_ANALYSIS_REQUEST_REPAIR_REVIEWED,
