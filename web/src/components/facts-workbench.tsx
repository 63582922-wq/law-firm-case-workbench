...
export function FactsWorkbench() {
  ...
  if (error) return <section className={styles.calculationBlocked}><p className={styles.eyebrow}>事实与争点</p><h3>{caseDataSourceConfig.kind === "persistent-disabled" ? "持久化模式未启用" : "案件快照未连接"}</h3><p>{error}</p><small>系统没有回退到另一套数据，也没有把未连接状态显示为成功。</small></section>;
  ...
  return <section className={styles.factsArea} aria-label="事实与争点台账">
    ...
    <p className={styles.resultHash}>案件快照 {review.snapshotHash.slice(0, 16)}…{review.transactionSnapshotHash ? `；交易快照 ${review.transactionSnapshotHash.slice(0, 16)}…` : ""}。修改上游材料或律师决定后，旧快照将失效。{review.requestId ? ` 请求号 ${review.requestId}` : ""}</p>
  </section>;
}
