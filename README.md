...
- 桌面包已携带自包含 Python/FastAPI 本机服务，由 Rust 父进程完成随机挑战、动态 loopback 端口、健康状态和退出联动监护。默认只报告“案件仍禁用”；只有已登记身份、显式专用预览库、Keychain 只读 AES 密钥及预创建私有加密目录四项同时满足时，sidecar 才装配受控持久化 API，不会因服务已启动而伪造律师身份或数据库连接。
...
- “身份与安全 → 模型与外部服务”现可配置 DeepSeek/通义千问百炼 Qwen3.5-OCR 的 API Key：密钥只通过 macOS 原生安全输入框进入系统钥匙串，页面仅显示是否已配置，不能填写任意供应商地址。模型 Key 不授予律师身份、案件访问或材料外发权限；真实供应商适配器、用量计量与逐案预授权界面仍未接通。
