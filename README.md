...
- 已实现律所 Ed25519 签发、30 天上限、设备秘密绑定、Keychain 双项目只读和会话装配的律师登记契约；没有真实签发服务、生产信任根和专用数据库时仍不启用登录或真实案件。
- 已实现登记、续期、远程撤销和“仅停用本机”的生命周期编排契约；签名登记凭证的保存仍只用合成 vault 测试，没有伪造律所签发服务。
- Tauri 桌面端已用原生凭证库 API 实现安装秘密初始化、回读复核、状态检查和仅删除本机登记凭证；秘密不进入命令参数或 WebView。实际 `.app` 已只读验证页面显示“尚未初始化”，本轮未写入用户 Keychain；律所激活按钮继续禁用。
...
- 官方案例数据已建立严格元数据目录，登记 6 个已核验的民间借贷真实案例候选；许可未建立前禁止批量抓取、全文仓库、商业训练和正式案件使用。
