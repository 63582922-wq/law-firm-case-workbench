...
# Keep LibreOffice out of the provider-egress Agent Worker image.  Only the
# isolated renderer (and the legacy API document worker when explicitly
# retained) opt into the larger rendering toolchain at build time.
...
RUN set -eux; \
    ...
    apt-get update; \
    packages="clamav poppler-utils fonts-arphic-uming fonts-arphic-ukai fonts-wqy-microhei"; \
...
PY
COPY api-gate.py /app/web_api_gate.py
