...
- 尚无真实律所签发/续期/撤销服务、生产信任根运营、MFA 或数据库迁移部署器；后端已实现签名登记、设备绑定、门限签名信任目录、无角色激活请求、同身份续期、远程撤销与本机停用接口；Tauri 已可通过原生 Keychain API 初始化/回读安装秘密、读取脱敏状态、只停用本机登记，并只从系统文件选择器导入经 sidecar 当前信任目录验签的 `.lawenroll`；但没有真实签发身份时不得装配为登录；
