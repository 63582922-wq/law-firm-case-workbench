...
- 尚无真实律所签发/续期/撤销服务、生产公钥轮换、签名凭证保存/恢复整链、MFA 或数据库迁移部署器；后端已实现签名登记、设备绑定、Keychain 只读、无角色激活请求、先验签后 CAS 保存、同身份续期、远程撤销与本机停用接口；Tauri 已可通过原生 Keychain API 初始化/回读安装秘密、读取脱敏状态和只停用本机登记；但没有真实签发身份时不得装配为登录；
...
- 桌面 `身份与安全` 页显示 sidecar 的 `NOT_ENROLLED/NOT_CONFIGURED` 和 Keychain 脱敏状态；用户可显式初始化安装秘密或只停用本机登记，但不能自选角色或创建凭证，律所激活在真实 issuer/生产公钥缺失时保持禁用，健康进程不会冒充已登录。
