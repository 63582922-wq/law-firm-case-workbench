...
状态：`已实现显式配置、依赖注入、Tauri 文件夹选择桥、受监护本机 API sidecar 与持久化依赖装配；生产持久化运行仍未开放`
...
任一条件不满足即阻断。前六项全部满足时，sidecar 以 `PersistentApiDependencies` 直接启动持久 API：所有 PostgreSQL Store、加密对象库、一次性文件访问许可、案卷文件夹授权、原件单页预览、Agent 执行审计和外部调用预授权账本必须一次装配，不能只开其中一部分。合成模式出现任何持久化配置也会阻断，避免配置残留让测试页面误连数据库。配置对象的文本表示不包含 DSN，日志和错误信息不得输出密码或完整连接串。
...
- 桌面壳已把 Python 3.12/FastAPI/uvicorn 编译为自包含 sidecar，并通过私有 stdin 随机挑战、回执摘要、动态 `127.0.0.1` 端口和父进程退出联动完成启动/监护；默认服务只开放无文档的禁用态 `/healthz`。已实现的持久化装配会在七项内部预览门全部满足时改为直接启动受控持久 API；仓库未配置真实信任根、Keychain 工件密钥、专用 PostgreSQL 或私有对象目录，因此当前不会打开案件路由；
