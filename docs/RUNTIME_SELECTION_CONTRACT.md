...
- 尚无真实律所签发/续期/撤销服务、生产公钥轮换、Keychain 初始化 UI、MFA 或数据库迁移部署器；后端已实现签名登记、设备绑定、Keychain 只读、一次性桌面启动凭证与短时 Bearer 契约，但没有真实签发身份时不得装配为登录；
...
- 桌面 `身份与安全` 页只读显示 sidecar 返回的 `NOT_ENROLLED` 与 `NOT_CONFIGURED`；该状态页不创建凭证、不写 Keychain，也不把健康进程冒充已登录。
