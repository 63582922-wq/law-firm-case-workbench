...
| 文书一致性审查 | 律师确认的字段表与批准文书快照 | 已实现确定性发现、哈希化持久记录与 QA 阻断链 | 只报告确认值缺失、冲突文本和来源缺口；不改写文书、不选择律师立场。持久层只保存已批准工作产物绑定、输入/输出/字段/来源哈希和发现代码；提交 QA 只接受覆盖完全相同 PDF 集合、当前版本且无阻断项的审查 |
