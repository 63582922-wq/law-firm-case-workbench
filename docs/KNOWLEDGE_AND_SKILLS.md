...
候选的律师确认使用独立 `agent-draft-review-v1` 哈希，绑定文书类型、候选内容、执行输入和唯一的 Skill/Tool。模型身份、Assistant 身份或不同哈希都不能确认；确认的对象才有资格在后续持久化命令中建立 Agent 提案。

通用 Agent 计划账本也在写入前拒绝 Assistant/模型登记 Word、Excel 起草 Tool；即使安装环境已启用隔离转换器，文书类提案只能由主办律师或复核律师建立。这是第二道门，不以客户端界面或模型自我声明替代权限检查。
...
| 文书一致性审查 | 律师确认的字段表与批准文书快照 | 已实现确定性发现、受控 Worker 投影、哈希化持久记录、版本差异内核与 QA 阻断链 | 只报告确认值缺失、冲突文本和来源缺口；不改写文书、不选择律师立场。标题、段落/标题、来源引用和章节顺序的内容差异只以位置与 SHA-256 投影，任何语义改变必须重新审批；获批的新语义文本会使同文书类型、同用途的旧不同版本自动过期，无法再进入提交包。专用 SYSTEM_WORKER 只接受已批准结构化文书快照并派生安全投影；持久层只保存已批准工作产物绑定、输入/输出/字段/来源哈希和发现代码；提交 QA 只接受覆盖完全相同 PDF 集合、当前版本且无阻断项的审查 |

这张表的“待接入”不是对用户假称可用：默认注册表会把它们标为 `GATED`，调用一律被拒绝，直到隔离 Worker、回读校验、受管候选件登记和回归样本都完成。起草 Tool 已不再暴露只有内存字节的 `create_docx_draft` / `create_xlsx_ledger`，而是只接受能返回“可编辑源件 + 断网渲染 PDF 预览 + 共同复核哈希”的 Tool。受信任桌面运行时仅在隔离转换器、加密对象库与复核对持久化已同时装配时，才可动态启用 Office 起草能力；浏览器、模型和普通配置文件均不能切换它。`agent_reviewable_draft_coordinator.py` 还会重算批准输入哈希、核对不可变 Agent 提案、先登记复核对再追加成功回执；`desktop_agent_draft_runtime.py` 进一步要求已登记身份、专用 Worker、持久账本和独立私有工作目录。sidecar 在开启案件路由前装配它，并仅在本机就绪诊断中报告是否已装配；该执行器不提供 HTTP 或 WebView 入口。当前可执行注册表位于 `backend/case_kernel/skill_registry.py`。
...
本机读取、PDF 规范化、确定性计算和已配置的公开研究不需要用户先提供模型 API Key。只有在接入需要付费模型的“长文抽取、归纳、起草”能力时，桌面端才需要由律所管理员配置供应商凭证；凭证进入系统钥匙串/受控配置，不进入案件文件夹、Git、提示词日志或导出材料。当前 macOS 桌面端在“身份与安全 → 模型与外部服务”提供 DeepSeek 与 Qwen 的原生安全输入和仅状态展示；它只完成 Keychain 配置，不能自行授权外发或证明供应商适配器已启用。
