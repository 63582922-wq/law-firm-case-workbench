...
| 文书一致性审查 | 律师确认的字段表与批准文书快照 | 已实现确定性发现、受控 Worker 投影、哈希化持久记录、版本差异内核与 QA 阻断链 | 只报告确认值缺失、冲突文本和来源缺口；不改写文书、不选择律师立场。标题、段落/标题、来源引用和章节顺序的内容差异只以位置与 SHA-256 投影，任何语义改变必须重新审批；专用 SYSTEM_WORKER 只接受已批准结构化文书快照并派生安全投影；持久层只保存已批准工作产物绑定、输入/输出/字段/来源哈希和发现代码；提交 QA 只接受覆盖完全相同 PDF 集合、当前版本且无阻断项的审查 |
