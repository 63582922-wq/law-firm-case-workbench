...
状态：`受控持久化路由已实现；默认禁用；commercial-production 仍需真实身份、专用 PostgreSQL 与人工放行证据`

## 隔离原则

- 本文件只定义律所受管的 PostgreSQL API。无需登记即可使用的 `local-standalone` 本机基础案卷是独立的 SQLite 元数据与只读盘点边界，不挂载本文件的事实、利息、文书、提交、原件预览、OCR 或外部请求路由；其完整约束见 [ADR-0025](../decisions/ADR-0025-local-first-standalone-workspace.md)；
- `case_api.app` 继续只服务 `alpha_*` 合成对象，不挂载任何持久化路由；
- `case_api.persistent_app.create_persistent_app()` 未注入依赖时只返回“已禁用”健康状态，案件路由不存在；
- 启用持久化路由必须使用受控 PostgreSQL 运行时（`postgres-internal-preview` 或经独立明确确认的 `commercial-production`）、受控 PostgreSQL 台账 Store 和服务端身份解析器；商用模式只接受名称以 `_production` 结尾的数据库，且不得携带 preview 确认开关。桌面 sidecar 还必须同时持有当前重新验签的登记身份、Keychain 只读 AES-256-GCM 密钥及预创建的私有加密对象目录。`desktop_persistent_runtime` 一次装配全部 Store 和访问 Broker，缺任一项即在监听案件路由前退出；
...
`/v1/matters/{matter_id}/document-consistency-reviews` 是文书一致性审查的只读快照和 SYSTEM_WORKER 记录面。记录请求只含已批准 PDF 工作产物 UUID、共同 `review_input_hash`、报告/字段/来源哈希和发现代码；接口不接收文书正文、律师确认值或提示词。持久层对每一个绑定重查同案已批准工作产物，并重算安全报告投影哈希；律师和普通 Agent 不能伪造 `PASS`。创建提交 QA 时还会重查该记录覆盖的工作产物集合、版本和 `blocking_count=0`，任一不一致返回阻断错误。未注入服务时接口返回 503，不把内存报告当作当前审查。
