...
实现状态：`0005_formal_calculations.sql` 与 `0006_legal_source_rules.sql` 已形成持久化控制链；正式计算只接收规则包标识/哈希，规则段及年利率由服务端读取。官方来源受控抓取、关键条款解析和 LPR 参数机器核验已完成首段；专用 PostgreSQL 实迁移和执业律师规则验收仍未完成。
...
民间借贷 2020 年过渡路径必须先由 `private_lending_transition_planner` 形成候选，而不是由模型或计算器自行补全：合同成立于 2020-08-20 前、且一审在该日后新受理的案件，必须分开审查历史期间与 2020-08-20 后期间；后一期间的候选上限锚定起诉时一年期 LPR 四倍，不按每年 LPR 自动滚动。合同成立于该日或之后的候选上限则锚定合同成立时一年期 LPR 四倍。受理日恰在边界日、日期缺失、或起诉/受理顺序不一致时，计划器必须阻断并要求律师确认。
...
`LPR_MULTIPLE` 必须同时绑定两类不同快照：一是证明“按 LPR 若干倍”法律公式的法律/司法解释快照，二是证明具体基准数值及日期的官方利率数据快照，并保存参数定位说明。两者不能复用同一快照；任一快照被替代、撤销许可或失去核验状态时，依赖规则与计算失效。具体一年期 LPR 只能由服务端在登记该官方快照时重新解析已认证加密原件后写入不可变观察记录；规则审批仅可选择观察记录的精确定位和倍数，禁止浏览器、模型或草稿提交基准利率数值。
